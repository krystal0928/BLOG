...
<script>
import { deleteArticleCollection, getArticleCollectionList } from '@/api/article-collection'
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getArticleCollectionList(this.query).then(res => {
        if (res.code === 200) {
          this.list = res.data
    ...
    handleDelete(id) {
      this.$confirm('此操作将彻底删除该文章收藏, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deleteArticleCollection(id).then(res => {
          if (res.code === 200) {
            this.$message.success(res.msg)
            this.fetchData()
          }
